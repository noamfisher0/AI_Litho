...
def run_evaluation(
    ...
    if issues:
        print(f"\n  {failed_images} failed, {timed_out} timed out. "
...
def parse_args():
    ...
    p.add_argument("--plot",        action="store_true",
                   help="Generate 4-metric overview plots.")
    ...
    p.add_argument("--aggregate",   action="store_true",
                   help="Re-aggregate results_per_image.csv -> results_average.csv.")
    p.add_argument("--workers",     type=int, default=None,
                   help="Worker processes (default: cpu_count - 1).")
    p.add_argument("--timeout",     type=int, default=120,
                   help="Per-image timeout in seconds (default: 120).")
    p.add_argument("--samples",     type=int, default=None,
                   help="Cap images per subset. Omit or pass None for all images.")
...
if __name__ == "__main__":
    ...
    if args.aggregate:
        ...
        if not per_image_csv.exists():
            print(f"results_per_image.csv not found. Run --evaluate first.")
        else:
    ...
    if args.plot_metric:
        ...
        if metric not in VALID_METRICS:
            ...
            save_dir = str(OUTPUT_DIR) if args.save_plots else None
            plot_single_metric(str(averaged_csv), metric=metric,
                               save_dir=save_dir,
                               filter_resolutions=filter_resolutions,
                               filter_methods=filter_methods,
                               filter_datasets=filter_datasets)

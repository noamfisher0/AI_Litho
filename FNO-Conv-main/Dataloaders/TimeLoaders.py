...
class IncompressibleBase(BaseTimeDataset):
    ...
    def __getitem__(self, idx):
        ...
        if not self.transpose:
            ...
            inputs = (torch.from_numpy(self.reader["velocity"][i + self.start, t1, 0:2])
                .type(torch.float32)
            )
            ...
            inputs = (
                torch.from_numpy(
                    np.transpose(self.reader["velocity"][i + self.start, t1, 0:2], axes = axes)
                )
                .type(torch.float32)
            ...
            label = (
                torch.from_numpy(
                    np.transpose(self.reader["velocity"][i + self.start, t2, 0:2], axes = axes)
                )
                .type(torch.float32)
                .reshape(2, self.resolution, self.resolution)
            )
...
class BrownianBridgeTimeDataset(IncompressibleBase):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.N_max = 20000
        self.data_path  =  self.data_path + "/brownian_bridge.nc"
        self.post_init()
...
class VortexSheetTimeDataset(BaseTimeDataset):
    def __init__(self, *args, **kwargs):
        ...
        self.N_max = 20000
        self.data_path  =  self.data_path + "/vortex_sheet.nc"
        self.reader = h5py.File(self.data_path, "r")
        self.post_init()
...
class SinesTimeDataset(BaseTimeDataset):
    def __init__(self, *args, **kwargs):
        ...
        self.N_max = 20000
        self.data_path  =  self.data_path + "/sines.nc"
        self.reader = h5py.File(self.data_path, "r")
        self.post_init()


class PiecewiseConstantsTimeDataset(IncompressibleBase):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.N_max = 20000
        self.data_path  =  self.data_path + "/pwc.nc"
...
class PiecewiseConstantsTimeDataset2(BaseTimeDataset):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        assert self.max_num_time_steps * self.time_step_size <= 20
...
class GaussiansTimeDataset(BaseTimeDataset):
    def __init__(self, *args, **kwargs):
        ...
        if self.in_dist:
            data_path = self.data_path + "/gaussians.nc"
        else:
            raise NotImplementedError()
